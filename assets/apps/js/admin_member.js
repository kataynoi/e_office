$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/admin_member/fetch_admin_member',
            data: {
...
crud .ajax = {
    del_data:function (id,cb){
        var url = '/admin_member/del_admin_member',
            params = {
                id: id
            }
        ...
        app.ajax(url, params, function (err, data) {
            err ? cb(err) : cb(null, data);
             ...
             var url = '/admin_member/save_admin_member',
                 params = {
                     items: items
                 }
             ...
             app.ajax(url, params, function (err, data) {
                 err ? cb(err) : cb(null, data);
                   ...
                   var url = '/admin_member/get_admin_member',
                       params = {
                           id: id
                       }
...
crud.set_after_update = function (items,row_id) {
    ...
    row_id.find("td:eq(0)").html(items.id);row_id.find("td:eq(1)").html(items.prename);row_id.find("td:eq(2)").html(items.name);row_id.find("td:eq(3)").html(items.cid);row_id.find("td:eq(4)").html(items.sex);row_id.find("td:eq(5)").html(items.position);row_id.find("td:eq(6)").html(items.hospcode);row_id.find("td:eq(7)").html(items.employee_type);row_id.find("td:eq(8)").html(items.group);row_id.find("td:eq(9)").html(items.username);row_id.find("td:eq(10)").html(items.email);row_id.find("td:eq(11)").html(items.password);row_id.find("td:eq(12)").html(items.user_mobile);row_id.find("td:eq(13)").html(items.date_in);row_id.find("td:eq(14)").html(items.date_out);row_id.find("td:eq(15)").html(items.user_level);row_id.find("td:eq(16)").html(items.address);row_id.find("td:eq(17)").html(items.active);row_id.find("td:eq(18)").html(items.driver);row_id.find("td:eq(19)").html(items.order);

}
crud.set_after_insert = function (items,id) {

            $('<tr name="row'+(id+1)+'"><td>'+id+'</td>' +
                '<td>' +items.id+'</td>' +'<td>' +items.prename+'</td>' +'<td>' +items.name+'</td>' +'<td>' +items.cid+'</td>' +'<td>' +items.sex+'</td>' +'<td>' +items.position+'</td>' +'<td>' +items.hospcode+'</td>' +'<td>' +items.employee_type+'</td>' +'<td>' +items.group+'</td>' +'<td>' +items.username+'</td>' +'<td>' +items.email+'</td>' +'<td>' +items.password+'</td>' +'<td>' +items.user_mobile+'</td>' +'<td>' +items.date_in+'</td>' +'<td>' +items.date_out+'</td>' +'<td>' +items.user_level+'</td>' +'<td>' +items.address+'</td>' +'<td>' +items.active+'</td>' +'<td>' +items.driver+'</td>' +'<td>' +items.order+'</td>' +
...
crud.set_update = function (data,row_id) {
    $("#row_id").val(row_id);
    $("#id").val(data.rows["id"]);$("#prename").val(data.rows["prename"]);$("#name").val(data.rows["name"]);$("#cid").val(data.rows["cid"]);$("#sex").val(data.rows["sex"]);$("#position").val(data.rows["position"]);$("#hospcode").val(data.rows["hospcode"]);$("#employee_type").val(data.rows["employee_type"]);$("#group").val(data.rows["group"]);$("#username").val(data.rows["username"]);$("#email").val(data.rows["email"]);$("#password").val(data.rows["password"]);$("#user_mobile").val(data.rows["user_mobile"]);$("#date_in").val(data.rows["date_in"]);$("#date_out").val(data.rows["date_out"]);$("#user_level").val(data.rows["user_level"]);$("#address").val(data.rows["address"]);$("#active").val(data.rows["active"]);$("#driver").val(data.rows["driver"]);$("#order").val(data.rows["order"]);
}
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.prename) { swal("กรุณาระบุคำนำหน้า");$("#prename").focus();}else if (!items.name) { swal("กรุณาระบุชื่อ สกุล");$("#name").focus();}else if (!items.cid) { swal("กรุณาระบุเลขบัตรประชาชน");$("#cid").focus();}else if (!items.sex) { swal("กรุณาระบุเพศ");$("#sex").focus();}else if (!items.position) { swal("กรุณาระบุตำแหน่ง");$("#position").focus();}else if (!items.hospcode) { swal("กรุณาระบุ");$("#hospcode").focus();}else if (!items.employee_type) { swal("กรุณาระบุประเภทพนักงาน");$("#employee_type").focus();}else if (!items.group) { swal("กรุณาระบุกลุ่่มงาน");$("#group").focus();}else if (!items.username) { swal("กรุณาระบุUsername");$("#username").focus();}else if (!items.email) { swal("กรุณาระบุE-Mail");$("#email").focus();}else if (!items.password) { swal("กรุณาระบุPassword");$("#password").focus();}else if (!items.user_mobile) { swal("กรุณาระบุโทรศัพย์");$("#user_mobile").focus();}else if (!items.date_in) { swal("กรุณาระบุ");$("#date_in").focus();}else if (!items.date_out) { swal("กรุณาระบุ");$("#date_out").focus();}else if (!items.user_level) { swal("กรุณาระบุประเภทผู้ใช้งาน");$("#user_level").focus();}else if (!items.address) { swal("กรุณาระบุที่อยู่");$("#address").focus();}else if (!items.active) { swal("กรุณาระบุสถานะการใช้งาน");$("#active").focus();}else if (!items.driver) { swal("กรุณาระบุพนักงานขับรถ");$("#driver").focus();}else if (!items.order) { swal("กรุณาระบุลำดับในกลุ่มงาน");$("#order").focus();}
    else{
        return true;
    }
